...
from model.layer_orth import GSOrth
...
class VGG(nn.Module):
    ...
    def __init__(self, features, len_feature=4608, num_classes=1000, initmethod='gaussian', **kwargs):
        super(VGG, self).__init__()
        self.features = features
        self.classifier = nn.Linear( len_feature, num_classes )
        self._initialize_weights( initmethod=initmethod )
...
def make_layers(cfg, batch_norm=False):
    ...
    for v in cfg:
        if v == 'M':
            ...
            """add orthogonalization layer"""
            if height == 16 and width == 16:
                layers += [ GSOrth(64, (height, width)) ]
                in_channels = 64
        else:
